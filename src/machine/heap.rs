...
impl<'a> ReservedHeapSection<'a> {
    ...
    pub(crate) fn pop_cell(&mut self) -> Option<HeapCellValue> {
        if self.heap_cell_len > 0 {
            self.heap_cell_len -= 1;
            self.pstr_vec.pop();

            unsafe {
                Some(ptr::read(self.heap_ptr.add(heap_index!(self.heap_cell_len)) as *const HeapCellValue))
            }
        } else {
            None
        }
...
impl Heap {
    ...
    */
    ...
    #[inline]
    pub(crate) fn null_byte_at(&self, loc: usize) -> bool {
        unsafe { ptr::read(self.inner.ptr.add(loc)) == 0u8 }
    }
